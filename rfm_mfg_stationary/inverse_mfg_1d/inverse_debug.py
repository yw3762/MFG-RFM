import torch
import numpy as np
from scipy.linalg import lstsq, pinv, inv
...
def inverse_HJB(models_u, models_b, collocs_b_cat, m_func, q_func, observed_u, F_l, Fb_avg, H_b, U_l, Mb, Ju, Jb, Qb,
    ...
    # M is of shape (Qb * Mb, Ju * Mu)
    M = build_RFM_matrix(models_u, q_vals, collocs_b_cat, Mb, Qb, Ju, eps)
    Mu = len(models_u)
    MTM_inv_f = inv(M.T @ M) @ Fb_avg
    v = MTM_inv_f / (1 + Fb_avg.T @ MTM_inv_f)

    # We modify the test slightly
    no_normalization_ = False
    if no_normalization_:
        Q_lb = F_l @ np.eye(Mu * Ju) @ pinv(M)
    else:
        I_bplus = np.eye(Mu * Ju) - np.outer(v, Fb_avg.T)
        Q_lb = F_l @ I_bplus @ pinv(M)

    if regularization:
        X = Q_lb @ H_b
        regularization_constant = 0.001
        tilde_theta_nj = torch.tensor(inv(X.T @ X + regularization_constant * np.eye(H_b.shape[1])) @ X.T @ (U_l - Q_lb @ W_b)).T
    else:
        tilde_theta_nj = torch.tensor(pinv(Q_lb @ H_b) @ (U_l - Q_lb @ W_b)).T
