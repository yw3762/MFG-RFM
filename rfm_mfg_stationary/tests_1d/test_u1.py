...
from utils.utils_1d import plot_RFM_1d, V
...
def pre_define(M_p, J_n, Q):
    ...
    for k in range(M_p):
        ...
        x_min = 1.0 / M_p * k  # interval l.b.
        x_max = 1.0 / M_p * (k + 1)  # interval u.b.
...
def cal_matrix(models, points, M_p, J_n, Q, eps=0.3):
    """
    ...
    for k in range(M_p):  # iterate over all partitions
        ...
        for m in range(M_p):  #
            ...
            if k == 0:
                A_1[0, m * J_n: (m + 1) * J_n] = values[0, :]
            elif k == M_p - 1:
                A_1[0, m * J_n: (m + 1) * J_n] -= values[Q, :]

            # normalization
            for i in range(Q):
                A_2[1, m * J_n: (m + 1) * J_n] += values[i, :]


        # get the true values for f's
        copy_pts = points[k].detach().numpy()
        true_f = V(copy_pts)
...
def test(models, M_p, J_n, Q, w, plot=False):
    ...
    numerical_values = []
    test_Q = int(1000 / M_p)
    plot_RFM_1d(models, w, "my plot", interval_length=1.0)
    for k in range(M_p):
        points = torch.tensor(np.linspace(1.0 / M_p * (k), 1.0 / M_p * (k + 1), test_Q + 1),
    ...
    print('L_inf=', epsilon.max(), 'L_2=', math.sqrt(1 * sum(epsilon * epsilon) / len(epsilon)))
    ...
    if plot == True:
        ...
        plt.show()
    return (epsilon.max(), math.sqrt(1 * sum(epsilon * epsilon) / len(epsilon)))



def main(M_p, J_n, Q, plot=False, moore=False, eps=0.3):
    # prepare models and collocation pointss
    models, points = pre_define(M_p, J_n, Q)

    # matrix define (Aw=b)
    A, f = cal_matrix(models, points, M_p, J_n, Q, eps)
