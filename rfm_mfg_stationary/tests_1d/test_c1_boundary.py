...
def solve_FP(models, collocs, q_func, dq_func, M_p, J_n, Q, eps=0.3, MMS_attempt=4):
    ...
    for k in range(M_p):
        for m in range(M_p):
            ...
            if k == 0:
                A_constraints[0, m * J_n: (m + 1) * J_n] += 0.5 * values[0, :]
            elif k == M_p - 1:
                A_constraints[0, m * J_n: (m + 1) * J_n] -= 0.5 * values[Q, :]
...
def solve_HJB(models, collocs, m_func, q_func, M_p, J_n, Q, eps=0.3, lam=0, MMS_attempt=5):
    ...
    for k in range(M_p):
        for m in range(M_p):
            ...
            # Periodicity constraint, evaluate on boundary
            if k == 0:
                A_constraints[0, m * J_n: (m + 1) * J_n] += 0.2 * values_hjb[0, :]
            elif k == M_p - 1:
                A_constraints[0, m * J_n: (m + 1) * J_n] -= 0.2 * values_hjb[Q, :]
